...
object FileUtil {
    ...
    fun getImage(fileName: String): Bitmap? {
        val filePath = baseImageDir + fileName
     ...
     * 获取图片uri
     */
    fun getImageFileUri(context: Context, fileName: String): Uri? {
        val filePath = baseImageDir + fileName
        if (createOrExistsFile(filePath)) {
            val file = getFileImage(fileName)
    ...
    fun getApkFileUri(context: Context, fileName: String): Uri? {
        val filePath = baseApkDir + fileName
        if (createOrExistsFile(filePath)) {
            val file = getFileImage(fileName)
            return if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
                Uri.fromFile(file)
    ...
    fun getFileApk(fileName: String): File {
        val imgPath = File(baseApkDir)
        return File(imgPath, fileName)
    }

    fun getFileCrashLog(fileName: String): File {
        val imgPath = File(baseCrashDir)
        return File(imgPath, fileName)
    }
